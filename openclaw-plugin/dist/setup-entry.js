...
export function resolveAccount(_cfg, accountId) {
    ...
    if (!entry)
        throw new Error(`hands-and-claws: account "${accountId}" not found in ~/.openclaw/hands-and-claws.json`);
    if (!entry.baseUrl)
        throw new Error(`hands-and-claws: account "${accountId}" missing baseUrl`);
    if (!entry.token)
        throw new Error(`hands-and-claws: account "${accountId}" missing token`);
