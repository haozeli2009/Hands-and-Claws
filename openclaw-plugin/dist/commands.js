...
export function parseCommand(text) {
    ...
    switch (name) {
        ...
        case "getlist": {
            ...
            if (!demand)
                return { type: "_help", text: "Usage: /getlist <demand>" };
        ...
        case "finish": {
            ...
            const cid = parts[0] ?? "";
            if (!cid)
                return { type: "_help", text: "Usage: /finish <cid> [demand_uid]" };
        ...
        case "join": {
            ...
            if (!room_id)
                return { type: "_help", text: "Usage: /join <room_id>" };
            return { type: "fetch_group", room_id };
        }
        case "msg": {
            const spIdx = args.indexOf(" ");
            if (spIdx === -1)
                return { type: "_help", text: "Usage: /msg <room_id> <text>" };
